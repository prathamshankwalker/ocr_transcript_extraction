...
def get_dataset_metrics(true_labels, pred_labels):
    ...
    for true_label, pred_label in zip(true_labels, pred_labels):
        ...
        if true_label != "OTHER":
            ...
            if pred_label != "OTHER":
                metrics_dict[true_label]["FP"] += 1

    df = pd.DataFrame()

    for field in metrics_dict:
        precision, recall, f1_score = performance(
            metrics_dict[field]["TP"], metrics_dict[field]["FP"], metrics_dict[field]["FN"])
        support = metrics_dict[field]["Support"]

        if field != "OTHER":
            temp_df = pd.DataFrame([[precision, recall, f1_score, support]], columns=[
                                   "Precision", "Recall", "F1-Score", "Support"], index=[field])
        ...
        for pred_file in os.listdir(pred_path):
            if (".tsv" in true_file) and (".tsv" in pred_file):
                if true_file == pred_file:
                    ...
                    true_labels, pred_labels = get_doc_labels(
                        true_file, pred_file)
...
if __name__ == "__main__":
    ...
    doc_true = "dataset/dataset/val_w_ann/boxes_transcripts_labels"
    # print(os.listdir("dataset/dataset/train"))
    doc_pred = "dataset/dataset/val_copy/boxes_transcripts"
